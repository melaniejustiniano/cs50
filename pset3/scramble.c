...
void draw(void)
{
    ...
    // line break to look like the CS50 version
    printf("\n  ");
    ...
    for (int i = 0; i < DIMENSION; i++)
        {
            ...
            // goes through columns while in one row
            for (int j = 0; j < DIMENSION; j++)
            {
            
                // prints characters in correct order
                printf("%c ", grid[i][j]);
    ...
    // line break to look like CS50 version
    printf("\n");
...
bool lookup(string word)
{
    ...
    while (low <= high)
    {
        ...
        if (strcmp(word, dictionary.words[middle].letters) == 0)
        {
        
            // does not recount points for found words
            ...
            // if the word is the middle word
            return true;
            
        }
        
        
        else if (strcmp(word, dictionary.words[middle].letters) > 0)
        {
        
            // the word is farther in the dictionary than the middle word
            // next time the while loop runs, search the bottom half
            low = middle + 1;
        ...
        else
        {
            ...
            // the word is earlier in the dictionary than the middle word
            // next time the while loop runs, search the top half
            high = middle - 1;
...
void scramble(void)
{
    ...
    // defines new grid to scramble first grid into
    char temporarygrid[DIMENSION][DIMENSION];
    
    // creates a scrambled grid as temporary grid
    for (int i = 0; i < DIMENSION; i++)
    {      
        ...
        for (int j = 0; j < DIMENSION; j++)
        {
            ...
            int dimensionsize = DIMENSION - 1;
            
            // defines which grid coordinates correspond with the temporary grid
            int x = j;
            int y = dimensionsize - i;
            
            // puts the actual grid values into temporary grid
            temporarygrid[x][y] = grid[i][j]; 
            
        }
        
    }
    
    // changes original grid to reflect scrambled temporary grid
    // must be a new for loop so as to avoid replacing unused characters
    for (int i = 0; i < DIMENSION; i++)
    {
        ...
        {   
            ...
            // sets values for grid as per temporary grid
            grid[i][j] = temporarygrid[i][j];
